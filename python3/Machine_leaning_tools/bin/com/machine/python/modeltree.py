...
def treeForeCast(tree, inData):
    if not isTree(tree): return modelTreeEval(tree, inData)
    if inData[tree['spInd']] > tree['spVal']:
        if isTree(tree['left']): return treeForeCast(tree['left'], inData)
        else: return modelTreeEval(tree['left'], inData)
    else:
        if isTree(tree['right']): return treeForeCast(tree['right'], inData)
        else: return modelTreeEval(tree['right'], inData)
...
def predata(trainfile,prefile,outfile,(m,n),splittype):
    ...
    for line in fr.readlines():
        ...
        predata = treeForeCast(mytree,mat(yhat))
        yhat = []
